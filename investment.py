...
def calculate_compound_interest(investment, rate, years):
    ...
    x=round(investment * ((1 + (rate / 100)) ** years), 2)
    print("Value is ", rate,years,x)
    return x
...
def get_investment_recommendations(investment_data, countries, investment_amount):
    ...
    for country in countries:
        ...
        for investment_type, rate in investment_data.items():
            adjusted_rate = adjust_for_inflation(rate, inflation_rate)
